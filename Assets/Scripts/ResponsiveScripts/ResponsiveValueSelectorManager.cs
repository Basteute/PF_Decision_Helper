...
public class ResponsiveValueSelectorManager : MonoBehaviour
{
    ...
    void Start()
    {
      ...
      selectorPanel.GetComponent<RectTransform>().anchoredPosition =
        new Vector2(mainPanelPos.x * 0.5f, mainPanelPos.y * 0.1f);

       // Change the size of the selector buttons
       for(int i = 2; i <= 14; i++)
       {
         float btnWidth = GameObject.Find("BtnCard" + i).GetComponent<RectTransform>().rect.size.x;
         float parentPanelHeight = selectorPanel.GetComponent<RectTransform>().rect.size.y;

         GameObject.Find("BtnCard" + i).GetComponent<RectTransform>().sizeDelta =
